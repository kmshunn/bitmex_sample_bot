...
class order():
    ...
    def get_pos_info(self):
        ...
        if position_list['currentQty'] == 0: # sizeが0より大きければ現在LONG状態、0より小さければ現在SHORT状態と判断
            ...
            side = 'SHORT'
        message = "Side:" + side + ", Size:" + round(position_list['currentQty']) + ", avgEntryPrice:" + position_list['avgEntryPrice']
        return message
...
class Bot:
    ...
    def getCandlestick(self, number, period):
        ...
        for i in periods:
            row = res[i]
            length = len(row)
            for columns in row[:length-(number+1):-1]:
                if columns[4] != 0:
                    column = columns[0:6]
                    data.append(column)
        ...
        while True:
            ...
            if pos == 0:
                ...
                if judgement[2] == 1:
                    ...
                    self.next_order_min = datetime.datetime.now() + datetime.timedelta(minutes = next_order_span)
                    print(datetime.datetime.now(), self.next_order_span)
                    self.order.market("sell", lot)
                    pos -= 1
                    plRange = Bid - lastPositionPrice
                    pl.append(pl[-1] + plRange * lot)

                    message = "Long close. size:{}, price:{}, pl:{}".format(lot, Bid, pl[-1])
                    print(message)
                    self.lineNotify(message)
                ...
                if judgement[3] == 1:
                    ...
                    pl.append(pl[-1] + plRange * lot)
                    message = "Short close. Lot:{}, Price:{}, pl:{}".format(lot, Ask, pl[-1])
                    print(message)
                    self.lineNotify(message)
            ...
            if datetime.datetime.now().minute % 5 == 0 and datetime.datetime.now().second < 5:
                print(message)
                self.lineNotify(message)
